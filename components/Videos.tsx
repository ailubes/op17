...
type VideosProps = {
  copy: HomeMessages["videos"];
};

export const Videos: React.FC<VideosProps> = ({ copy }) => {
  const videos = videoMeta.map((video, index) => ({
    ...video,
    title: copy.items[index]?.title || "",
  }));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 group cursor-pointer reveal opacity-0 translate-y-10 transition-all duration-1000">
            <div className="relative aspect-video bg-slate-800 border border-white/10 overflow-hidden shadow-2xl">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <span className="inline-block px-3 py-1 bg-ukraine-blue text-white font-barlow font-bold text-xs uppercase tracking-wider mb-2">
                  {copy.featuredBadge}
                </span>
                <h3 className="font-bebas text-4xl text-white drop-shadow-lg">
                  {copy.featuredTitle}
                </h3>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-6">
            {videos.map((video, index) => (
              <div
                key={video.id}
                className="flex gap-4 group cursor-pointer reveal opacity-0 translate-y-10 transition-all duration-1000"
              ...
              >
                <div className="relative w-40 aspect-video bg-slate-800 overflow-hidden border border-white/5 shrink-0">
                  ...
                  />
                  <div className="absolute bottom-1 right-1 bg-black/80 px-1.5 py-0.5 text-[10px] font-mono text-white">
                    {video.duration}
                  </div>
                </div>
                <div className="flex flex-col justify-center">
                  ...
                  <span className="font-barlow text-slate-500 text-sm uppercase tracking-wider">
                    {video.views} {copy.viewsLabel}
                  </span>
                </div>
              </div>
            ))}
            ...
            <div className="mt-auto pt-6 border-t border-white/10 reveal opacity-0 translate-y-10 transition-all duration-1000 delay-500">
              <a
                href="#"
                className="flex items-center gap-2 font-barlow font-bold text-gold uppercase tracking-widest hover:text-white transition-colors"
              >
                {copy.watchAll} <span className="text-xl">-&gt;</span>
              </a>
            </div>
          </div>
        </div>
