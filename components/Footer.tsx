...
type FooterProps = {
  copy: HomeMessages["footer"];
};

export const Footer: React.FC<FooterProps> = ({ copy }) => {
  return (
    <footer className="bg-slate-950 pt-32 pb-14 border-t border-white/5">
      <div className="container mx-auto px-6 md:px-14">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-24">
          ...
          <div>
            <h4 className="font-barlow font-bold uppercase tracking-widest mb-8 text-white">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mb-6">{copy.newsletterText}</p>
        ...
        <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 text-sm text-slate-500 font-inter">
          <p>&copy; {new Date().getFullYear()} Oleh Plotnytskyi. {copy.rights}</p>
          <div className="flex items-center gap-4">
            <span className="font-barlow font-bold tracking-[0.2em] text-gold text-lg">
              {copy.slava}
            </span>
          </div>
        </div>
