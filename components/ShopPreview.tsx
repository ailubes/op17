...
export const ShopPreview: React.FC<ShopPreviewProps> = ({ copy }) => {
  const products = productFrames.map((frame, index) => ({
    ...frame,
    title: copy.products[index]?.title || "",
    price: copy.products[index]?.price || "",
  }));
              ...
              <div className="group-hover:scale-110 group-hover:-rotate-6 transition-transform duration-700">
                ...
                {product.type === "jersey" && (
                  <svg width="180" height="220" viewBox="0 0 180 220" fill="none" className="drop-shadow-2xl">
                    ...
                    <text x="45" y="140" fill="white" className="font-bebas text-[60px]">
                      17
                    </text>
                  </svg>
                )}
                {product.type === "hoodie" && (
                  <svg width="180" height="220" viewBox="0 0 180 220" fill="none" className="drop-shadow-2xl">
                    <path
                      d="M30 40C30 40 50 10 90 10C130 10 150 40 150 40L170 80V200H10V80L30 40Z"
                      fill="#1E293B"
                    />
                    <path d="M70 110L90 130L110 110" stroke="#FFD500" strokeWidth="4" />
                    <text x="55" y="170" fill="rgba(255,255,255,0.2)" className="font-bebas text-[30px]">
                      THUNDER
                    </text>
                  </svg>
                )}
                {product.type === "ball" && (
                  <div className="w-40 h-40 bg-gold rounded-full relative shadow-[inset_-20px_-20px_40px_rgba(0,0,0,0.3)] flex items-center justify-center">
                    ...
                    <span className="font-bebas text-slate-900 text-xl rotate-[-10deg] font-bold">
                      Plotnytskyi
                    </span>
                  </div>
                )}
