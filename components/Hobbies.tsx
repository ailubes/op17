...
type HobbiesProps = {
  copy: HomeMessages["hobbies"];
};

export const Hobbies: React.FC<HobbiesProps> = ({ copy }) => {
  return (
            ...
            <div className="flex flex-wrap gap-8 mt-12 pt-10 border-t border-white/10">
              {copy.stats.map((stat, index) => (
                <div key={index} className="text-center">
                  <div className="font-bebas text-4xl text-gold">{stat.value}</div>
                  <div className="font-barlow font-bold text-slate-400 uppercase tracking-widest text-xs">
                    {stat.label}
                  </div>
                </div>
              ))}
